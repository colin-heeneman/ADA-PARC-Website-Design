...
server <- function(input, output, session) {
  ...
  observe({
    ...
    default <- if ("People with Disabilities" %in% choices) "People with Disabilities" else choices[1]
  ...
  observe({
    if (!is.null(input$national_category_selector) && !is.null(input$national_variable_selector)) {
    ...
    national_data_table_friendly <- national_data %>%
      ...
      mutate(
        across(
          ...
          ~ scales::percent(.x, accuracy = 0.01, scale = 1)
        )
      ) %>%
...
server <- function(input, output, session) {
 ...
 output$city_summary <- renderUI({
    ...
    race_ethnicity <- city_place() %>% transmute(
      White = create_number_percent_string(pwd_white_nonhisp, pwd_white_nonhisp_pct),
      `Black/African American` = create_number_percent_string(pwd_black, pwd_black_pct),
      `Hispanic/Latino` = create_number_percent_string(pwd_hisp, pwd_hisp_pct),
      Asian = create_number_percent_string(pwd_asian, pwd_asian_pct),
      `Some other race(s) identified` = create_number_percent_string(pwd_multiple, pwd_multiple_pct)
    ) %>% 
    ...
    sex <- city_place() %>% transmute(
      Male = create_number_percent_string(pwd_male, male_total_pwd_pct),
      Female = create_number_percent_string(pwd_female, female_total_pwd_pct)
    ) %>% 
    ...
    disability_types <- city_place() %>% transmute(
      ...
      `Ambulatory (difficulty walking or climbing stairs)` = create_number_percent_string(pwd_ambulatory, pwd_ambulatory_pct),
      `Self Care (difficulty bathing or dressing)` = create_number_percent_string(pwd_selfcare, pwd_selfcare_pct),
      `Independent Living (difficulty doing errands alone)` = create_number_percent_string(pwd_indliving, pwd_indliving_pct)
    ) %>%
    ...
    employment <- city_place() %>% transmute(
      `Living below poverty (as of 2023, yearly income below $14,891 for individual or $29,60 for family of four)` = create_number_percent_string(pwd_below_poverty, pwd_below_poverty_pct),
      `Employed (part or full time)` = create_number_percent_string(pop_total_employed_16_plus, pwd_employed_subj),
      `Out of the labor force (not working and not actively seeking employment)` = create_number_percent_string(pwd_not_in_labor_force, pwd_not_employed_subj)
    ) %>% 
    ...
    education <- city_place() %>% transmute(
      `Less than high school` = create_number_percent_string(pwd_lessthan_highschool, pwd_lessthan_highschool_pct),
      `High school or GED equivalent` = create_number_percent_string(pwd_highschoolequiv, pwd_highschoolequiv_pct),
      `Associate degree or some college` = create_number_percent_string(pwd_degree_aa, pwd_degree_aa_pct),
      `Bachelor’s degree or higher` = create_number_percent_string(pwd_degree_grtoeq_ba, pwd_degree_grtoeq_ba_pct)
    ) %>%
      pivot_longer(everything(), names_to = "Indicator", values_to = "Value (%)") %>%
      mutate("Bullets" = paste0('<li>', Indicator, " &mdash; ", `Value (%)`, '</li>')) %>%
      pull(Bullets) %>%
    ...
    city_summary_tables <- paste0(
      ...
      '<p style="font-style: italic; color: #000000;">Percentages displayed in the "Disability Demographics" section refer to the % of the total population of selected city which both have a disability, and are a part of the given demographic.</p>',
...
tags$div(
  ...
  # Embed the PDF file from Google Drive
  tags$iframe(
    style = "height: 750px; width: 100%; border: none; overflow-y: scroll;", 
    src = "https://drive.google.com/file/d/11nWM_BwMWY5QxRwSZvR9r-pH8CBPQli2/preview"
  )
)
```
