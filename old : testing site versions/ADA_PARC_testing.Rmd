  ...
  tags$main(role = "main", 
    ...
    tags$section(`aria-labelledby` = "section-table-of-selected-topic", 
      div(class = "table-container",
        div(style = "overflow-x: auto; width: 100%;",  # << updated
...
server <- function(input, output, session) {
  ...
  # Observe category change and update topic selector
  ...
  # Ensure default selections populate on app load
  observe({
    if (is.null(input$national_variable_selector)) {
      choices <- names(national_variable_choices())
      default <- if ("People with Disabilities" %in% choices) "People with Disabilities" else choices[1]
      updateSelectInput(session, "national_variable_selector",
                        choices = choices,
  ...
  }, server = FALSE)  # << important: not server-side rendering
}
...
server <- function(input, output, session) {
 ...
 output$city_summary <- renderUI({
    ...
    race_ethnicity <- city_place() %>% transmute(
      ...
      `Black/African American` = create_number_percent_string(pwd_black, pwd_black_dis_pct),
      `Hispanic/Latino` = create_number_percent_string(pwd_hisp, pwd_hisp_dis_pct),
      Asian = create_number_percent_string(pwd_asian, pwd_asian_dis_pct),
      `Some other race(s) identified` = create_number_percent_string(pwd_other_multiple, pwd_other_multiple_dis_pct)
    ) %>% 
